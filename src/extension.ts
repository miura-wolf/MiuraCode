...
/**
 * Check if we should auto-open the MiuraCode sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening MiuraCode sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the MiuraCode sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the MiuraCode Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize MiuraCode Cloud service.
